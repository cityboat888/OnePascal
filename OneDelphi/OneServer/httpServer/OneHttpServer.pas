...
function GetContentType(QFileType: string): string;
...
begin
  // vFileType :
  Result := '';
  vFileType := QFileType.toUpper;
  // Content-Type
  if vFileType = '.HTML' then
    Result := 'text/html;charset=UTF-8'
  ELSE
    if vFileType = '.JPG' then
    Result := 'image/jpeg'
  ELSE
    if vFileType = '.PNG' then
    Result := 'image/png'
  ELSE
    if vFileType = '.GIF' then
    Result := 'image/gif'
  ELSE
    if vFileType = '.TXT' then
    Result := 'text/plain'
  ELSE
    if vFileType = '.CSS' then
    Result := 'text/css;charset=UTF-8'
  ELSE
    if vFileType = '.JS' then
