...
uses
  ...
  OneHttpControllerRtti,OneNeonHelper,OneStreamString,
  OneRttiHelper, OneHttpConst, OneControllerResult, OneMultipart;
...
type
  ...
  private
    ...
    FEncodingZip:Boolean;
  public
    destructor Destroy; override;
    procedure SetUrlParams();
    procedure SetHeadParams();
    procedure SetInContent(qInContent: Tobject);
  public
    ...
    property RequestInHeaders: THttpHeader read FRequestInHeaders write FRequestInHeaders;
...
procedure THTTPCtxt.SetInContent(qInContent: Tobject);
var
  vBytes:TBytes;
  vBody:RawByteString;
...
begin
  ...
  begin
    // 不做处理到自已单元处理
    vBody:=StreamToStringDeCode(TStream(qInContent));
    FRequestInContent := UTF8Decode(vBody);
  end
  ...
  begin
    vBody:=StreamToStringDeCode(TStream(qInContent));
    vBytes := TEncoding.UTF8.GetBytes(vBody);
